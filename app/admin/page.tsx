...
if (error) setMessage("❌ Error al actualizar nota: " +
error.message);
else {
...
const actualizarEstudiante = async (id: string, nombre: string,
telefono: string | null) => {
const { error } = await supabase
.from("estudiantes")
.update({ nombre, telefono })
.eq("id", id);
if (error) setMessage("❌ Error al actualizar estudiante: " +
error.message);
else {
...
router.push("/login");
} else if (data.user.email !== "andres.pedrozab@uniagustiniana.edu.co") {
...
if (loading) return <p className="text-center mt-10">Cargando
datos...</p>;
return (
<div className="max-w-7xl mx-auto mt-10 p-6 space-y-10">
<h1 className="text-3xl font-bold text-center">Panel
Administrativo</h1>
...
<tr key={act.id}>
<td className="border
p-2">{nombreEstudianteDe(act.estudiante)}</td>
<td className="border
p-2">{nombreCursoDe(act.curso)}</td>
<td className="border
p-2">{act.titulo}</td>
<td className="border
p-2">{act.tipo}</td>
...
{act.imagen && (
<img src={act.imagen}
alt="actividad" className="w-20 h-20 object-cover" />
)}
</td>
...
<button
onClick={() =>
actualizarNota(act.id, act.nota ?? 0)}
